...
The PyPI wheel does not ship a `.wsgi` file. Create
`/opt/cmdbsyncer/app.wsgi` with the following content — it is what
`WSGIScriptAlias` points at in the vhost below:

```python
import os
os.chdir('/opt/cmdbsyncer')
...
The `os.chdir` line is what lets the Syncer pick up `local_config.py`
and the `plugins/` folder from `/opt/cmdbsyncer`.
